...
| 20 | **stdin 이 열린 채 입력 대기 고착** — ① `claudex exec` 를 백그라운드 task 로 실행 ② **인자 검증보다 stdin 읽기(`$(cat)`)가 앞선 래퍼 스크립트**(2026-09-07 실측: `deft-review nosuch` 20분+ 무응답 — 검증 명령 자체가 조용히 멈춰 테스트가 끝나지 않음) | stdin 이 열린 채 입력 대기 고착 — 50분간 CPU 0.1초, MCP 서버 미기동, 보드 무진행 (실측) | L3 는 **포그라운드 + `</dev/null`** 로 실행. 백그라운드가 필요하면 stdin 닫음 필수 + etime/cputime 으로 고착 감시 |
...
| 22 | **orca 모드 + `~/.local/bin/cmux` 구버전 shim 잔재 (2026-07-24 실측)** | Orca 터미널에서 bare `cmux` 가 소켓으로 **별도 실행 중인 cmux 앱**에 연결돼 엉뚱한 pane 을 조용히 조작(오발사). shim 이 ALL_HELPERS 에 없고 gap-fill 은 부재 시에만이라 구 shim 이 영원히 미갱신 | deft-bin-sync(claude-2.48.1~)가 기본 동기 시 **deft-cmux-shim 마커 보유 파일만** 갱신(진짜 cmux CLI 는 마커 검사로 보호). orca 검증은 ① 환경 판정 ORCA_* 우선 ② rebalance 계열 no-op·shim exit 1 확인 ③ spawn 은 `deft-claudex-native-spawn` orca 분기(split `--command` 원샷·`env:"orca"` JSON·tui-idle)·pane 은 종료 시 Orca 자동 close. **orca tmux shim 실측(추가)**: `tmux`=`~/.orca/claude-agent-teams-bin/tmux`('3.4' 표방, 소스 `claude-agent-teams-tmux-compat.js`) — **claude 팀원 pane(team.panes) 한정** list-panes/send-keys/capture-pane/split-window/select-pane/kill-pane/last-pane/display-message 실구현. resize-pane no-op·geometry/`#{pane_dead}`/`#{pane_pid}` 빈 값·swap-pane/list-windows 미지원. orca terminal 직접 생성 pane(claudex 워커)은 shim 비대상 → orca terminal read/send/wait/close 경로. 🚨 **팀원 pane 을 `orca terminal close` 로 닫으면 shim 레지스트리 stale → 이후 Agent spawn `terminal_exited`**(실측) — 복구는 `tmux kill-pane -t <stale id>`, 팀원 pane 정리는 tmux kill-pane 만 |
| 23 | **장수 orca 세션 팀 spawn 전면 차단 (2026-07-31 실측 · 재발 2026-09-07)** — ⚠️ **표면 문자열이 가변**: `terminal_handle_stale` / `Failed to create teammate pane: Timed out waiting for the Orca runtime to respond` / `Failed to send command to pane %N`. 문자열이 아니라 **「장수 세션 + Agent spawn 실패」로 판정**할 것 | 오래 산 `orca claude-teams` 세션에서 `Agent` spawn 이 `Failed to create teammate pane: tmux: terminal_handle_stale` 로 전부 실패 (팀 생성 자체 불가 → NTP 경로 차단). shim `tmux split-window` 직접 호출도 동일 재현. ⚠️ **env `ORCA_TERMINAL_HANDLE` 이 유효해도 실패** — shim RPC(`agentTeams.tmuxCompat`)는 `{teamId, token, TMUX_PANE}` 만 동봉, 핸들·pane-key 미전송이라 클라이언트 상태와 무관(실측 반증 완료). `orca terminal list` 의 `term_*` 핸들은 조회마다 회전(leaf id 만 안정)하므로 **스냅샷 비교로 유효성 판정 금지 — read 등 실조작으로만** | Orca 본체 결함: 팀 상태의 등록 시점 leader 핸들 스냅샷이 remint 로 무효화돼 고착 (일반 orchestration 경로엔 pane-key remint 폴백 PR #7514 이 있으나 tmuxCompat 경로만 누락). **진단 시도 불요 — 즉시 multi-round MCP 버스 폴백**(claudex 워커는 `orca terminal` 직접 경로라 무관 — 작업 모드 버스 강등으로 완주 실증) 또는 세션 재시작(레지스트리 재구성). 예방: 팀 작업은 세션 초반에 시작. 업스트림: 이슈 stablyai/orca#11739 · 수정 PR #11742 (bluehansl 기여) |
...
| 24 | **codex code_mode_only 모델 — 버스 MCP 도구 top-level 미노출 (2026-08-03 실측)** | claudex 워커(gpt-5.6-sol)가 `[bus] 메시지 확인` 노크를 받고도 실제 도구 호출 없이 "ACK: 메시지 확인했습니다" 텍스트만 출력하고 턴 종료 — board 무진행, Lead 는 무응답 판정. 원인: `~/.codex/models_cache.json` `tool_mode` — gpt-5.6-sol/terra/luna 는 `code_mode_only`(MCP 도구가 exec 내부 `tools.mcp__bus__*` nested 로만 노출), gpt-5.5 는 `null`(direct). 프로토콜이 board 첫 메시지로 전달되는 구조라 첫 check 실패 시 지침도 못 읽는 부트스트랩 공백이 증폭. NTP `send_message` 는 `non_code_mode_only` 기본 true → DirectModelOnly 라 무관(소스 확정) | claude-2.50.0/codex-1.25.0 삼중 안전: ① spawn 모델 고정(`deft-claudex-native-spawn` 이 `-m` deft-model codex 기본 gpt-5.5 자동 주입, `DEFT_CODEX_MODEL` 오버라이드) ② 노크 문구·버스 도구 설명에 exec 호출 표면 명시 ③ 페르소나 §도구 호출 표면(tool_mode 표+예제). 검증 시 code_mode_only 경로는 `DEFT_CODEX_MODEL=gpt-5.6-sol` 로 spawn 해 board 응답 게시 여부 확인 (RATIONALE R-17). **E2E 실측 검증 완료(2026-08-03)**: L3 headless — `claudex exec -m gpt-5.6-sol` + 버스 인라인 주입에 **새 노크 문구만** 프롬프트로 주자 exec 경유 check_messages → post_message(reply_to 명시) 완주(board 실측). 대조군 gpt-5.5(direct) 도 동일 문구에서 회귀 없음 |

| 25 | **`BashOutput` deprecated — 스킬에 적으면 안 되는 도구 (2026-09-07 실측)** | background 로 밀린 작업의 출력을 회수하려 `BashOutput` 을 페르소나에 지시했으나 이 하네스에선 폐기됨 | 백그라운드 작업은 **출력 파일 경로를 반환**하고 `<task-notification>` 이 같은 경로를 준다 → **`Read` 가 정식 경로**. §1 의 "새 외부 명령은 실재 검증" 규칙을 **도구 이름에도** 적용할 것 (`ToolSearch select:<name>` 으로 확인) |
| 26 | **동형 결함은 엔진마다 확인 (2026-09-07 실측)** | codex 의 trusted directory 문제(`--skip-git-repo-check`)를 고쳤으나 **gemini 도 동일**하게 막힘 — `not running in a trusted directory` 승인 대기로 **무한 hang**(300초 초과 background 이동) + `--approval-mode plan` 이 default 로 강등 | gemini 는 `--skip-trust`. 한 엔진의 실행 계층 결함을 고치면 **나머지 엔진에서 동형을 반드시 확인**. ※ 이 결함은 같은 릴리즈의 stderr 보존 수정 덕에 드러났다 — `2>/dev/null` 이면 빈 출력만 남는다 |
| 27 | **timeout ≠ 작업 종료 (2026-09-07 실측)** | Bash timeout 은 작업을 죽이지 않고 background 로 옮긴다 — 프로세스는 계속 살아 출력이 자란다(실측: 60초 timeout 후 **174초 더 실행해 완주**, exit 0, 150KB→173KB) | 그러므로 timeout 보고를 "실패"로 처리해 종료 신호를 보내면 **거의 완성된 결과가 폐기**된다(R-18 사고). 오케스트레이터는 완료/진행중을 **센티널로 구분**할 것. 반대로 검증 시엔 이 성질을 이용해 **짧은 timeout 으로 경로를 결정적으로 강제**할 수 있다 |
