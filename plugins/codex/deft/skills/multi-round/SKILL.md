...
> 본 skill은 **Codex 포팅본**입니다. Claude Code용 동일 skill은 `plugins/deft/skills/multi-round/` 에 있습니다. 기본 워크플로는 동일하지만 사용자 데이터 경로·plugin cache 경로·pane 환경 외부 fallback에 차이가 있습니다.
...
| 참가자 구성 | 1순위 | 2순위 | 3순위 |
|---|---|---|---|
| **AI mix 또는 전원 claudex/codex** (pane 환경 — cmux/orca) | **MCP 버스** (본 아키텍처 — orca 모드는 워커 기동·노크만 orca 분기, §환경 판정) | send/capture 폴백 (§Phase 3-B — orca 등가 있음) | `multi-check` 사용 안내 후 중단 |
| **전원 Claude** (Lead 까지 Claude — Codex 측 skill 에선 드묾) | Claude Code 환경으로 안내 (팀메이트 기능은 Claude 전용) | MCP 버스 | `multi-check` 사용 안내 후 중단 |
| pane 환경 외부 (`DEFT_ENV=none`) | claudex MCP conversation (§Phase 3-C — stateful 지속 대화) | `multi-check` 사용 안내 후 중단 | — |

- 하위 순위로 내려가는 조건: 상위 경로의 전제(버스 스크립트·node·pane 환경)가 충족되지 않을 때. 내려갈 때마다 사용자에게 사유 1줄 보고.

## 환경 판정 — cmux vs orca (전 Phase 공통·최우선)

deft 스킬은 두 pane 환경을 지원한다: **cmux**와 **Orca**(stablyai/orca). pane 조작이 나오는 모든 절차 전에 아래 판정을 먼저 수행한다. (Lead=codex/claudex 라도 Orca 관리 터미널 안이면 ORCA_* 변수가 상속된다.)
...
if [ -n "${ORCA_WORKTREE_ID:-}" ] || [ -n "${ORCA_TERMINAL_HANDLE:-}" ]; then
  DEFT_ENV=orca      # Orca 터미널 (ORCA_* 변수는 Orca 가 세팅)
...
- 🚨 **orca 모드에서 cmux CLI 호출 전면 금지** — 에러가 나는 게 아니라 **별도 cmux 앱의 엉뚱한 pane 을 조용히 조작**한다(오발사, 실측). deft bin 은 orca 를 자체 인지한다 — 버스 노크(cmuxKnock)는 orca 경로(`orca terminal send`)로 자동 분기하고, rebalance 계열(cmux-rebalancing/-guard)·deft-cmux-shim 은 no-op/차단한다. **Lead 가 직접 치는 bare `cmux`/`tmux` 명령도 동일하게 금지**다.
...
| pane 비율 조정 | `cmux-rebalancing` | **불가** — Orca 는 resize CLI 미지원. "pane 비율은 UI 드래그로 조정" 안내로 대체 |
| pane 정리 | `cmux close-surface` / `tmux kill-pane` | `orca terminal close --terminal <handle>` (실측 — ptyKilled 포함) |

- 세부 플래그·핸들 규약은 **`orca skills get orca-cli`** 가 버전 일치 단일 소스 — orca 명령 실행 전 확인한다.
- Lead 자신의 터미널 핸들은 `$ORCA_TERMINAL_HANDLE` — Lead register 의 `--surface` 로 그대로 쓴다(버스가 `term_*` 핸들을 orca 노크로 처리).
- 이하 본문의 cmux 명령 서술은 **cmux 모드 기준**이다. orca 모드에서는 본 대응표와 각 Phase 의 `🟠 orca 모드` 분기를 따른다.
...
```bash
# A. 참가자 CLI 확인 + 환경 판정 (§환경 판정 — ORCA 우선)
HAVE_CLAUDE=0; HAVE_CLAUDEX=0; HAVE_CODEX=0
...
DEFT_ENV=""
if [ -n "${ORCA_WORKTREE_ID:-}" ] || [ -n "${ORCA_TERMINAL_HANDLE:-}" ]; then
  DEFT_ENV=orca   # ⚠️ ORCA 먼저 — orca 안에서도 cmux identify 가 성공해 오판되므로 순서 필수 (§환경 판정)
fi
...
if [ -z "$DEFT_ENV" ]; then
  if which cmux >/dev/null 2>&1 && cmux identify >/dev/null 2>&1; then DEFT_ENV=cmux; else DEFT_ENV=none; fi
fi
...
- **`DEFT_ENV` + `HAVE_BUS` 조합이 Phase 3 통신 경로를 결정** (§통신 우선순위 매트릭스):
  ...
  - `DEFT_ENV=orca && HAVE_BUS=1` → **Phase 3-A orca 분기** (§3-A 🟠 — 워커 기동은 `orca terminal split --command` 원샷, 노크는 버스가 `term_*` 핸들로 orca send)
  - `DEFT_ENV=cmux && HAVE_BUS=0` → Phase 3-B (send/capture 폴백)
  - `DEFT_ENV=orca && HAVE_BUS=0` → Phase 3-B orca 등가 (orca terminal send/read/wait)
...
**엔진 상대 강점 참고 (배정 휴리스틱 — 강제 아님)**: 번갈아 배정이 기본이되, 주제·분담 성격이 뚜렷하면 아래를 참고해 페르소나↔엔진을 맞춘다. 사용자가 엔진 구성을 명시하면 항상 그것이 우선. (일반 관찰 기반 휴리스틱 — 측정된 사실 아님. 회의 모드·작업 모드 공통 참고.)

| 엔진 | 상대 강점 |
|---|---|
| claude | 구현·파일 수정·도구 활용, 신호/보고 프로토콜 준수 |
| claudex/codex | 코드 리뷰·아키텍처 비평·리팩토링 관점 — 이종 교차 검증 가치가 큼 |

예: 작업 모드에서 "구현 mate + 검증 mate" 분담이면 구현=claude, 검증=claudex 배정이 기본 추천.
...
>   ```
> - **(3) rebalancing skip** — Orca 는 resize CLI 미지원(bin 가드 no-op). pane 비율은 UI 드래그 안내. focus 복원도 불요.
> - **(5) 등록·페르소나**: `register --surface "$W_HANDLE"` (orca 노크 대상). 페르소나 부트는 `orca terminal send --terminal "$W_HANDLE" --text "$PERSONA_BOOT" --enter`.
> - codex 업데이트 프롬프트 스킵 입력도 `orca terminal send --terminal "$W_HANDLE" --text "3" --enter` 등가.
...
WORKER_CMD="claude --model "$(deft-model claude 2>/dev/null||echo claude-fable-5)" --dangerously-skip-permissions --strict-mcp-config --mcp-config $SESSION_DIR/mcp-$WORKER_NAME.json --allowedTools mcp__bus__check_messages,mcp__bus__post_message,mcp__bus__list_participants"
cmux send --surface "$W1_SURFACE" "$WORKER_CMD"
cmux send-key --surface "$W1_SURFACE" Enter
```
...
- 🟠 **orca 모드 등가 (§환경 판정 대응표)**: TUI 기동은 `orca terminal split --command "<기동명령>"` 원샷 / prompt 주입 `orca terminal send --terminal <handle> --text "<한 줄>" --enter`(sanitize 동일 적용) / 응답 감지는 idle-stable 폴링 대신 **`orca terminal wait --for tui-idle --timeout-ms <N>` → `orca terminal read`(`--cursor`/`--limit`) 2단 권장** — orca 의 tui-idle 판정이 agent CLI idle 을 내장 인식해 더 정확·빠르다.
...
> **산출물 라우팅 — 독자(audience) 기준**: `board.jsonl`/`summary.md`/`transcript.md` 는 **회의 세션 기록(내부)** — SESSION_DIR 유지. 그러나 회의 결론을 **파일 형태의 대외 완성물**(사용자 보고서·타팀 공유 문서)로 낼 때는 plugin-data 가 아니라 — work-id 가 티켓(`IT-\d+`)이면 `~/.ai/tickets/<work-id>/`, 프로젝트·OSS 작업이면 해당 repo 에 저장하고, summary.md 에 그 경로 링크를 남긴다. (판별 질문: "팀 바깥 독자가 읽는 완성물인가?") 사용자 환경의 상위 지침(전역/프로젝트 AGENTS.md·CLAUDE.md)이 별도 산출물 경로를 규정하면 **그것이 우선**한다 — 본 규칙은 기본값이다.
...
**소유권 (필수)**: cmux/orca 는 **다중 워크스페이스·다중 세션** 환경이다. **본 회의가 (2)에서 분할해 추적한 `W*_SURFACE`(cmux) / `W*_HANDLE`(orca) 만** 닫는다. 다른 워크스페이스/세션의 pane·surface·handle 을 추측으로 닫지 말 것 — **전체 순회·와일드카드 close 금지**.
...
- 🟠 **orca 모드**: 위 cmux/tmux 정리 명령 대신 **추적한 `W*_HANDLE` 로만** `orca terminal close --terminal "$W_HANDLE"` (실측 — ptyKilled 포함). rebalancing 복원 단계는 없음(resize 미지원).
...
| 6 | Lead surface 캡처는 `cmux identify` 의 `.caller.surface_ref` 사용 (orca 모드는 `$ORCA_TERMINAL_HANDLE`) | fallback 미작동 |
...
| 셋 다 미설치 | abort |
| node 미설치 또는 버스 헬퍼 없음 (`HAVE_BUS=0`) | Phase 3-B (send/capture) 폴백 + 사유 1줄 보고 (orca 모드는 orca terminal 등가 — §3-B 🟠) |
