...
> - 메인 세션의 단 하나의 목표는 **사용자 질의에 즉각 반응할 수 있는 idle 상태 유지**다. 실제 능동 작업(팀원 spawn·페르소나 생성·파일 편집)을 **실행하는 순간에만** 바쁘고, **팀원 응답·구현 완료를 기다리는 구간**은 메인을 점유하지 않는다.
> - 🚫 **대기를 foreground 로 구현 절대 금지** (긴 `sleep` 루프·동기 폴링). 팀원 응답은 `<teammate-message>` 자동 주입으로 **다음 턴**에 온다 — Lead 가 능동 sleep 으로 기다리지 말고, 대기가 필요하면 `run_in_background`(`&`)로 던지고 메인은 즉시 턴을 끝낸다.
>
> **원칙 2 — 출력은 "고정된 announce 지점"에서만. 단계 중계 전면 금지.**
> - 사용자는 스킬을 **직접 의도해서** 호출했다 — 상세 진행 보고가 불필요하다. Lead 가 대화에 말하는 지점을 **아래로 고정**하고, 그 사이 모든 단계는 **말없이 연속 실행**한다(여러 spawn 을 한 턴에 묶어 처리, 중간에 한 줄씩 말하지 말 것):
>
>   | # | 유일한 announce 지점 | 예시 |
>   |---|---|---|
>   | 1 | **팀 구성 예고** — 요청 + 인원 + 각 역할을 **한 번에** | "백엔드·프론트·QA 3명으로 팀을 구성해 IT-14610 코딩 작업을 시작합니다." |
>   | 2 | **스폰 완료** | "팀원 3명 스폰 완료 — 요건 분석을 시작합니다." |
>   | 3 | **단계 전환** (요건분석→구현→검증, 각 1회) + 단계 게이트 사용자 컨펌 | "구현 완료 — 검증 단계로 넘어갑니다." |
>   | 4 | **중간/최종 결과** | "리뷰 통과 — 변경 3파일·테스트 12건." |
>   | 5 | **사용자 개입 필요 시**(BLOCKED·게이트 승인 등) | 무엇이 필요한지만 |
>
> - 🚫 **위 외 일체 출력 금지** — "첫 팀원 spawn / 두 번째 추가 / pane 기록 / N번째 기동" 같은 spawn 단계별 나레이션 금지. spawn 은 1번 예고 후 **전부 한 번에 조용히** 실행하고 2번에서 완료만 알린다.
> - 🚫 **내부 메커니즘 출력 금지**: pane 분할·페르소나 주입·헬퍼 동기화·레이아웃 정렬·readiness·team-id·rebalance·"Ran N shell commands".
...
- teammate가 활성화된 하니스에서 실행 — **`cmux claude-teams` 또는 `orca claude-teams`** 양쪽 지원. Orca 는 Agent Teams 용 tmux shim(`~/.orca/claude-agent-teams-bin/tmux` → `orca agent-teams-tmux`)을 PATH 에 주입해 팀 spawn 의 pane 시각화를 자동 처리한다(실측 Orca 1.4.150).
...
#   (실측 — 조용한 오발사), cmux identify 성공 여부를 먼저 보면 orca 안에서 cmux 모드로 오판된다.)
DEFT_ENV=""
if [ -n "${ORCA_WORKTREE_ID:-}" ] || [ -n "${ORCA_TERMINAL_HANDLE:-}" ]; then
  DEFT_ENV=orca      # orca claude-teams — pane 시각화 자동(tmux shim). cmux CLI 호출 전면 금지
fi
...
echo "deft 환경: $DEFT_ENV"   # none 이면 §0-2 fallback
...
> **`cmux-rebalancing`**: Lead 와 팀원 컬럼 비율을 정책대로 재조정하는 헬퍼. **전체 팀원 spawn 후 1회** 호출 (§2-3). **cmux 모드 한정** — orca 모드는 호출하지 않는다(Orca 는 resize CLI 미지원. 호출돼도 내장 가드가 no-op — pane 비율은 UI 드래그로 조정 안내).

### 0-2. 팀 pane 하니스 외부 실행 시 (fallback — `DEFT_ENV=none`)

팀원 시각화(pane 자동 분할, §2-2)는 `cmux claude-teams` 또는 `orca claude-teams` 환경에서 동작한다. **어느 쪽도 아닌 환경**(`DEFT_ENV=none`)에서 실행 중이면:

- **기본: 단일 Claude 모드로 degrade** — 팀 spawn을 skip하고 Lead 단독으로 §4 단계 게이트를 진행한다. 의견 토론·합의가 목적이면 `deft:multi-round`를 권유한다.
- **권장 안내: `cmux claude-teams`(또는 Orca 환경이면 `orca claude-teams`) 재시작** — 팀 운영·시각화가 필요하면 사용자에게 재시작을 권한다.
...
| `Agent` (`name`, `model:"fable"`) | 팀원 spawn (역할별 페르소나 주입). **`team_name` 인자는 deprecated(무시됨) — 넣지 않는다** |
...
### 2-2. pane 분할 — 자동 (cmux/orca 공통)

사용자는 Claude Code를 `cmux claude-teams` 또는 `orca claude-teams`로 실행한다. 두 환경 모두 `Agent` spawn은 **pane 분할까지 자동 처리**된다 — cmux 는 claude-teams 하니스가 직접, Orca 는 tmux shim(`~/.orca/claude-agent-teams-bin/tmux` → `orca agent-teams-tmux`)이 팀 spawn 의 tmux 호출을 네이티브 pane 분할로 변환한다(실측). Lead가 별도 분할 명령을 호출할 필요 없다. (어느 쪽도 아니면 §0-2 fallback)

> **팀원 pane 관찰·개입 (orca 모드 포함)**: 팀원 pane 의 화면 읽기·직접 입력이 필요하면 **tmux 문법이 orca 모드에서도 그대로 유효**하다 — shim 이 팀원 pane 한정으로 `capture-pane`/`send-keys`(-l·특수키 변환)/`select-pane`/`last-pane`/`display-message` 를 실구현한다(실측). 단 `resize-pane` 은 no-op, `#{pane_width}` 등 geometry 변수·`#{pane_dead}`/`#{pane_pid}` 는 빈 값(cmux shim 과 동일 특성), `swap-pane`/`list-windows` 미지원.

### 2-3. pane 비율 재조정 — spawn 과 함께 rebalance 워처 발사 (cmux 모드 한정)

> 🟠 **orca 모드 (§0-1 환경 판정)**: 본 절 전체를 **skip** 한다 — rebalance 자체가 불가 확정: `orca terminal resize` 부재 + tmux shim 의 `resize-pane` 이 **no-op**이고 `#{pane_width}` 등 geometry 변수도 빈 값(소스 실측 — 측정·조정 모두 불가). 워처/guard 는 호출돼도 내장 가드가 no-op. pane 배치는 orca claude-teams 가 자동으로 하며, **비율이 마음에 안 들면 UI 드래그로 조정하라고 사용자에게 안내**한다. `LEAD_REF`/`BASE` 캡처도 하지 않는다(무의미 — rebalance 없음).
...
└── (팀 내부 부산물만 — 통신 임시물·스크립트 등. 대외 완성물은 §3-4-1 라우팅)
```
...
> `work.md`의 `## 작업 계획` 체크리스트가 **유일한 진행 상태 소스**다(누락 방지).

#### 3-4-1. 산출물 라우팅 — 독자(audience) 기준 (필수)

파일을 생성하기 전 판별 질문 한 줄: **"팀 바깥 독자(사용자 또는 타팀)가 읽는 완성물인가?"**
...
팀원 모두 **Claude Fable 5 (`fable`)**. **`model:"fable"` 명시** — 팀원은 Lead의 모델을 상속하지 않으므로(공식 문서) 반드시 명시한다(미지정 시 기본값도 `fable` 이나 명시를 원칙으로). enum: `sonnet`/`opus`/`haiku`/`fable`.
...
Agent(
  ...
  model: "fable",                    # ← Fable 5. 팀원은 Lead 모델 미상속이라 명시
...
팀원이 완료 보고 → Lead가 git diff로 계획 대비 확인 → 일치하면 work.md 체크리스트 `[O]` + 역할별 섹션 취합; 불일치/이슈면 재작업 요청. 리뷰 결과는 `## REVIEW`. 테스트 코드는 사용자 요청 시에만 작성. 취합 결과가 **대외 완성물**(사용자 보고·타팀 공유 문서)이면 §3-4-1 라우팅대로 티켓 디렉토리/repo 에 배출하고 work.md 에 링크를 남긴다.
...
**① graceful 종료**: `SendMessage(to="<팀원이름>", message={type:"shutdown_request", reason:"…"})` → **`shutdown_approved`/`teammate_terminated` 가 자동주입될 때까지 기다린다**. graceful 은 느릴 수 있음(공식 "Shutdown can be slow" — 6초+, 느리면 수십 초).
- 🚨 **`message` 는 반드시 구조화 객체(`{type:"shutdown_request"}`) — 평문 종료 요청 절대 금지 (실측 사고 2026-06-25)**: "정리하고 종료해 주세요" 같은 **평문 문자열**로 보내면 팀원은 그걸 *일반 메시지*로 받아 **보고만 하고 프로세스는 안 내려간다**(팀원이 "shutdown 은 shutdown_response(approve:true)를 받아야 내려간다"며 올바르게 거부 — 실측). claude 팀원(in-process)은 **kill 도 금지**라 구조화 `shutdown_request` 가 **유일한 종료 수단**이다 — 평문으로 보내면 영영 안 죽는다. 안 죽었으면 kill 이 아니라 **구조화 shutdown_request 를 다시** 보낸다.
...
> 🟠 **orca 모드**: 아래 tmux 블록은 **orca 모드에서도 유효**하다 — orca claude-teams 의 tmux shim('tmux 3.4' 표방, `~/.orca/claude-agent-teams-bin/tmux`)이 **팀원 pane(team.panes) 한정으로** `list-panes`/`kill-pane` 을 실구현한다(실측 — 소스: Orca.app `claude-agent-teams-tmux-compat.js`). 금지는 **cmux CLI 만**(오발사). claude 팀원은 in-process 라 ①의 graceful 종료가 유일 수단인 것은 동일하고, 정상 종료 시 pane 은 Orca 가 정리한다 — 본 블록은 비정상 종료 안전망.
> - 🚨 **팀원 pane 을 `orca terminal close` 로 닫지 말 것 (실측 함정)** — shim 의 team.panes 레지스트리와 어긋나 stale 등록이 남고, **이후 `Agent` spawn 이 `tmux: terminal_exited` 로 깨진다**(stale pane 을 anchor 로 split 시도). 팀원 pane 정리는 **본 블록의 `tmux kill-pane`(shim — 레지스트리 동기)만** 사용한다. 이미 어긋났으면 복구: `tmux list-panes -a` 로 stale id 확인 → `tmux kill-pane -t %N`(stale 등록 제거 — 실측 검증) → spawn 재시도. `orca terminal close` 는 orca terminal 로 직접 만든 pane(claudex 워커 등) 전용.
```bash
...
```
> ⚠️ cmux/orca 양쪽 환경의 `tmux` 는 호환 shim 이라 `#{pane_dead}`/`#{pane_pid}` 는 **빈 값**을 반환한다(양쪽 실측) — pane 생사 판정에 쓸 수 없다. 그래서 세션앵커 `pgrep`(프로세스 생사) + `tmux list-panes`(pane 존재) + `kill-pane`(양쪽 shim 지원 확인됨)로만 판정한다. 정상 graceful 종료(팀원이 `shutdown_response{approve:true}` 호출) 시엔 하니스가 pane 을 자동으로 닫으므로 ② 는 force-kill 등 비정상 종료의 안전망이다.

**③ 레이아웃 복원** (cmux 모드 한정 — 🟠 orca 모드는 skip: resize·focus CLI 미지원/금지. `deft-log ... DONE` 기록만 남긴다) — 팀원 pane 을 닫은 뒤(부분 종료 포함) 남은 컬럼·row 를 정렬하고 focus 를 Lead 로 복원한다(multi-round §5-B 와 동일 패턴). 부분 종료가 흔한 agent-teams 에서 빈 행 흡수·focus 튐을 정리한다:
```bash
...
```
0. Phase 0 환경 검증 (§0) — 실험 플래그·버전·환경 판정(orca/cmux/none — ORCA_* 우선). none 이면 §0-2 fallback, orca 면 cmux CLI·rebalance 전면 금지(§2-3 🟠)
